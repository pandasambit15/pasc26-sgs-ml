#!/usr/bin/env python3
"""
Task Weight Optimization Experiment - Complete with Checkpointing
Supports flexible method selection: Manual, Uncertainty, GradNorm, DWA

Usage:
    python task_weight_optimization_complete.py --config config.yaml --methods Manual Uncertainty GradNorm DWA
    python task_weight_optimization_complete.py --config config.yaml --methods Uncertainty GradNorm  # Run subset
    python task_weight_optimization_complete.py --config config.yaml --resume  # Resume interrupted
"""
...
class UncertaintyWeighting(nn.Module):
    """
    Uncertainty-based Multi-Task Learning Weighting
    https://arxiv.org/abs/1705.07115

    Learns task weights as log(σ²) to balance homoscedastic uncertainty.
    """
    def __init__(self, n_tasks: int = 5, initial_log_vars: Optional[List[float]] = None):
        super().__init__()
    ...
    def compute_weighted_loss(self, losses: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict]:
        """
        Compute uncertainty-weighted loss with regularization.
        Loss: L_i / (2σ_i²) + log(σ_i)
        """
        ...
        for i, task in enumerate(task_names):
            if task in losses:
                weighted_loss += 0.5 * precision[i] * losses[task] + 0.5 * self.log_vars[i]
                weights_dict[task] = precision[i].item()

        return weighted_loss, weights_dict
...
class GradNormWeighter:
    ...
    Balances tasks by normalizing gradient magnitudes relative to inverse training rates.
    """
    def __init__(self, n_tasks: int = 5, alpha: float = 1.5, initial_weights: Optional[List[float]] = None):
    ...
    def get_weights(self) -> torch.Tensor:
        ...
        return F.softmax(self.weights, dim=0) * self.n_tasks

    def update_weights(self, losses: Dict[str, torch.Tensor], shared_params: nn.Parameter,
                      optimizer: torch.optim.Optimizer):
        """Update task weights based on gradient magnitudes."""
        task_names = ['viscosity', 'diffusivity', 'richardson', 'regime', 'positivity']

        # Convert losses dict to tensor
        loss_tensor = torch.stack([losses[task] for task in task_names if task in losses])

        # Initialize on first call
        if self.initial_losses is None:
            self.initial_losses = loss_tensor.detach()
            self.loss_ratios = torch.ones_like(loss_tensor)
            return
        ...
        # Compute gradients for each task - just measurements, fully detached
        grad_norms = []
        for i, task in enumerate(task_names):
            ...
            if grad is None:
                grad_norms.append(1e-8)
            else:
                grad_norms.append(grad.detach().norm().item())  # Pure Python float

        # Convert to tensor for computation
        grad_norms = torch.tensor(grad_norms, dtype=torch.float32, device=self.weights.device)
        mean_grad_norm = grad_norms.mean()

        # Compute target gradient norms
        loss_ratios_tensor = torch.tensor(loss_ratios.cpu().numpy(),
                                         dtype=torch.float32,
                                         device=self.weights.device)
        target_grad_norms = mean_grad_norm * (loss_ratios_tensor ** self.alpha)

        # Compute L1 loss between current and target grad norms
        # This has gradients w.r.t. self.weights through the softmax in get_weights()
        # But we need to connect it to weights somehow...

        # Actually, let's use a direct weight update rule instead of backprop
        # Increase weight if grad_norm < target, decrease if grad_norm > target
        with torch.no_grad():
            # Compute adjustment: if grad too small, increase weight; if too large, decrease
            grad_ratio = target_grad_norms / (grad_norms + 1e-8)

            # Update weights directly (simple multiplicative update)
            self.weights.data = self.weights.data * (0.9 + 0.1 * grad_ratio)

            # Normalize to keep weights in reasonable range
            self.weights.data = self.weights.data - self.weights.data.mean()
            self.weights.data = torch.clamp(self.weights.data, min=-2.0, max=2.0)
...
class DWAWeighter:
    ...
    Adjusts task weights based on rate of change of task losses.
    """
    def __init__(self, n_tasks: int = 5, temperature: float = 2.0,
                 initial_weights: Optional[List[float]] = None, device: str = 'cuda'):
        self.n_tasks = n_tasks
        ...
        if initial_weights is None:
            ...
            self.weights = torch.tensor(initial_weights, device=device)
    ...
    def update_weights(self, losses: Dict[str, torch.Tensor], epoch: int):
        """Update task weights based on loss rate of change."""
        task_names = ['viscosity', 'diffusivity', 'richardson', 'regime', 'positivity']

        # Convert losses dict to tensor ON DEVICE
        loss_tensor = torch.stack([losses[task].detach() for task in task_names if task in losses])
        loss_tensor = loss_tensor.to(self.device)

        if self.prev_losses is None or epoch < 2:
            self.prev_losses = loss_tensor
            self.weights = torch.ones(self.n_tasks, device=self.device)
            return

        # Ensure prev_losses is on correct device (critical for resume!)
        if self.prev_losses.device != self.device:
            self.prev_losses = self.prev_losses.to(self.device)

        # Compute loss ratios (current / previous)
        loss_ratios = loss_tensor / (self.prev_losses + 1e-8)

        # Apply softmax with temperature
        exp_weights = torch.exp(loss_ratios / self.temperature)
        self.weights = self.n_tasks * exp_weights / exp_weights.sum()
    ...
    def state_dict(self):
        ...
        return {
            ...
            'prev_losses': self.prev_losses.cpu() if self.prev_losses is not None else None,
    ...
    def load_state_dict(self, state_dict):
        ...
        self.n_tasks = state_dict['n_tasks']
        # CRITICAL: Move loaded tensors back to correct device!
        self.weights = state_dict['weights'].to(self.device)
        if state_dict['prev_losses'] is not None:
            self.prev_losses = state_dict['prev_losses'].to(self.device)
        else:
            self.prev_losses = None
...
class TaskWeightExperiment:
    """Manages task weight optimization experiments with checkpointing."""
    ...
    def train_manual(self, train_loader, val_loader, test_loader,
        ...
        self.logger.info(f"Training {self.architecture} with Manual weights (seed={seed})")
        ...
        # Create model
        model = create_model_from_architecture(self.architecture)
        ...
        # Checkpoint path
        run_checkpoint = self.output_dir / f"checkpoint_Manual_{self.architecture}_seed{seed}.pt"
        start_epoch = 0
        ...
        for epoch in range(start_epoch, self.config['epochs']):
            ...
            # Track weight history (constant for manual)
            ...
            if val_loss < best_val_loss:
                ...
                # Save best model
                best_model_path = self.output_dir / f"best_model_Manual_{self.architecture}_seed{seed}.pt"
                torch.save({
    ...
    def train_uncertainty(self, train_loader, val_loader, test_loader, seed: int = 42) -> Dict:
        ...
        self.logger.info(f"Training {self.architecture} with Uncertainty weighting (seed={seed})")
        ...
        # Checkpoint path
        run_checkpoint = self.output_dir / f"checkpoint_Uncertainty_{self.architecture}_seed{seed}.pt"
        start_epoch = 0
        ...
        for epoch in range(start_epoch, self.config['epochs']):
            ...
            if val_loss < best_val_loss:
                ...
                # Save best model
                best_model_path = self.output_dir / f"best_model_Uncertainty_{self.architecture}_seed{seed}.pt"
                torch.save({
                    'model_state_dict': model.state_dict(),
    ...
    def train_gradnorm(self, train_loader, val_loader, test_loader, seed: int = 42) -> Dict:
        ...
        self.logger.info(f"Training {self.architecture} with GradNorm (seed={seed})")
        ...
        # Create GradNorm weighter
        weighter = GradNormWeighter(
            n_tasks=5,
            alpha=self.config.get('gradnorm_alpha', 1.5)
        )

        # Get shared parameter for gradient computation
        # Use a parameter from the feature encoder (truly shared across tasks)
        # Instead of the last parameter which might be task-specific
        model_params = list(model.parameters())
        # Use parameter from first half of model (shared layers)
        shared_param = model_params[len(model_params)//4]  # Use a middle-layer parameter
        ...
        # IMPORTANT: Create optimizer BEFORE moving to device to keep leaf status
        weight_optimizer = optim.Adam(
            [weighter.weights],
            lr=self.config.get('weight_learning_rate', 0.025)
        )

        # NOW move weights to device (optimizer already knows about the parameter)
        ...
        # Checkpoint path
        run_checkpoint = self.output_dir / f"checkpoint_GradNorm_{self.architecture}_seed{seed}.pt"
        start_epoch = 0
        ...
        if run_checkpoint.exists():
            ...
            # Recreate optimizers with loaded state
            model_optimizer.load_state_dict(checkpoint['model_optimizer_state_dict'])
            weight_optimizer.load_state_dict(checkpoint['weight_optimizer_state_dict'])
        ...
        for epoch in range(start_epoch, self.config['epochs']):
            ...
            for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
                ...
                # Step 1: Update GradNorm weights FIRST (needs clean computation graph)
                weighter.update_weights(losses, shared_param, weight_optimizer)

                # Step 2: Get UPDATED weights
                current_weights = weighter.get_weights()

                # Step 3: Compute weighted loss with updated weights
                total_loss = sum(current_weights[i] * losses[task]
                               for i, task in enumerate(['viscosity', 'diffusivity', 'richardson', 'regime', 'positivity']))

                # Step 4: Update model with weighted loss
                model_optimizer.zero_grad()
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                model_optimizer.step()
            ...
            if val_loss < best_val_loss:
                ...
                # Save best model
                best_model_path = self.output_dir / f"best_model_GradNorm_{self.architecture}_seed{seed}.pt"
                torch.save({
                    'model_state_dict': model.state_dict(),
    ...
    def train_dwa(self, train_loader, val_loader, test_loader, seed: int = 42) -> Dict:
        ...
        self.logger.info(f"Training {self.architecture} with DWA (seed={seed})")
        ...
        # Create DWA weighter with device parameter
        ...
        # Checkpoint path
        run_checkpoint = self.output_dir / f"checkpoint_DWA_{self.architecture}_seed{seed}.pt"
        start_epoch = 0
        ...
        for epoch in range(start_epoch, self.config['epochs']):
            ...
            # Update DWA weights based on epoch average losses
            ...
            if val_loss < best_val_loss:
                ...
                # Save best model
                best_model_path = self.output_dir / f"best_model_DWA_{self.architecture}_seed{seed}.pt"
                torch.save({
                    'model_state_dict': model.state_dict(),
    ...
    def run_all_experiments(self, data_dir: Path, scaler_dir: Path, methods: List[str],
        ...
        for method in methods:
            ...
            self.logger.info(f"\n{'='*60}")
            self.logger.info(f"Starting experiments for {method}")
            ...
            # Save results for this method (even if empty)
        ...
        # Generate summary (will skip failed methods)
    ...
    def plot_weight_evolution(self):
        ...
        # Only include methods that have successful runs
        for method in self.results.keys():
            if method != 'Manual' and len(self.results[method]) > 0:
                # Check if at least one seed has results
                has_results = any(
        ...
        if not methods_to_plot:
            self.logger.warning("No methods with weight evolution to plot (only Manual completed)")
            return
        ...
        for i, method in enumerate(methods_to_plot):
            # Plot all seeds for this method
            for seed_key in self.results[method].keys():
                ...
                for j, (task, title) in enumerate(zip(tasks, task_titles)):
                    ...
                    # Plot this seed's weights for this task
        ...
        plt.tight_layout()
        plot_path = self.output_dir / 'weight_evolution_comparison.png'
    ...
    def save_results_summary(self):
        ...
        for method in self.results:
            if len(self.results[method]) == 0:
                self.logger.warning(f"No results for {method} - skipping in summary")
                continue

            for seed_key in self.results[method]:
                result = self.results[method][seed_key]

                # Check if this result has the required data
                if 'test_metrics' not in result:
                    self.logger.warning(f"No test metrics for {method} {seed_key} - skipping")
                    continue
        ...
        if not rows:
            self.logger.error("No results to save - all methods failed!")
            return
...
def main():
    parser = argparse.ArgumentParser(description='Task Weight Optimization Experiments')
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(config_yaml.get('exp2_output_dir', 'experiments/task_weights'))

    # Build experiment config
    exp_config = {
        'epochs': config_yaml['training'].get('epochs', 100),
        ...
        'early_stopping_patience': config_yaml['training'].get('early_stopping_patience', 15),
        'gradnorm_alpha': config_yaml['training'].get('gradnorm_alpha', 1.5),
        'dwa_temperature': config_yaml['training'].get('dwa_temperature', 2.0)
    }

    # Create experiment
    experiment = TaskWeightExperiment(args.architecture, exp_config, output_dir, resume=args.resume)
